...
char defaultgamename[MAX_OSPATH] = "id1";
...
static void COM_Game_f(void)
{
	if (Cmd_Argc() > 1)
	{
		...
		if (*p2)
		{
			...
			if (!q_strcasecmp(p, defaultgamename)) {
		...
		if (q_strcasecmp(p, defaultgamename)) //game is not id1
		{
			if (*p2) {
...
void COM_InitFilesystem(void) //johnfitz -- modified based on topaz's tutorial
{
	...
	COM_AddGameDirectory(com_basedir, defaultgamename);
