...
void Sbar_DrawScrollString(int x, int y, int width, const char* str)
{
	...
	scale = Clamp(scr_sbarscale.value, 1.0, (float)vid.width / 320.0);
	left = x * scale;
	if (cl.gametype != GAME_DEATHMATCH)
		left += (((float)vid.width - 320.0 * scale) / 2);

	glEnable(GL_SCISSOR_TEST);
	glScissor(left, 0, width * scale, vid.height);
...
void Sbar_DrawFrags(void)
{
	...
	for (i = 0, x = 184; i<numscores; i++, x += 32)
	{
		...
		// brackets
		if (fragsort[i] == cl.entityindex - 1)
		{
...
void Sbar_Draw(void)
{
	...
	w = Clamp(scr_sbarscale.value * 320.0f, 320.0f, (float)vid.width);
	if (sb_lines && vid.width > w)
	{
		if (scr_sbaralpha.value < 1)
			Draw_TileClear(0, vid.height - sb_lines, vid.width, sb_lines);
		if (cl.gametype == GAME_DEATHMATCH)
			Draw_TileClear(w, vid.height - sb_lines, vid.width - w, sb_lines);
		else
		{
			Draw_TileClear(0, vid.height - sb_lines, (vid.width - w) / 2.0f, sb_lines);
			Draw_TileClear((vid.width - w) / 2.0f + w, vid.height - sb_lines, (vid.width - w) / 2.0f, sb_lines);
		}
	}
	//johnfitz
...
void Sbar_MiniDeathmatchOverlay(void)
{
	...
	scale = Clamp(scr_sbarscale.value, 1.0, (float)vid.width / 320.0); //johnfitz

	//MAX_SCOREBOARDNAME = 32, so total width for this overlay plus sbar is 632, but we can cut off some i guess
	if (vid.width/scale < 512 || scr_viewsize.value >= 120) //johnfitz -- test should consider scr_sbarscale
		return;
