...
void SCR_Conwidth_f(cvar_t* var)
{
	...
	vid.conwidth = Clamp(vid.conwidth, 320, vid.width);
...
void SCR_ScreenShot_f(void)
{
	...
	glReadPixels(0, 0, vid.width, vid.height, GL_RGB, GL_UNSIGNED_BYTE, buffer);

	// now write the file
	if (!q_strncasecmp(ext, "png", sizeof(ext)))
		ok = Image_WritePNG(imagename, buffer, vid.width, vid.height, 24, false);
	else if (!q_strncasecmp(ext, "tga", sizeof(ext)))
		ok = Image_WriteTGA(imagename, buffer, vid.width, vid.height, 24, false);
	else if (!q_strncasecmp(ext, "jpg", sizeof(ext)))
		ok = Image_WriteJPG(imagename, buffer, vid.width, vid.height, 24, quality, false);
	else
		ok = false;
...
==================
SCR_TileClear
johnfitz -- modified to use vid.width/vid.height instead of vid.width/vid.height
...
void SCR_UpdateScreen(void)
{
	...
	//GL_BeginRendering(&0, &0, &vid.width, &vid.height);
