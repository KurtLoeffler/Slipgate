...
void BoundPoly(int numverts, float* verts, vec3_t* mins, vec3_t* maxs)
{
	...
	mins->x = mins->y = mins->z = 999999999;
	maxs->x = maxs->y = maxs->z = -999999999;
	v = verts;
...
void SubdividePolygon(int numverts, float* verts)
{
	...
	for (i = 0; i<3; i++)
	{
		m = (mins.values[i] + maxs.values[i]) * 0.5;
		m = gl_subdivide_size.value * floor(m/gl_subdivide_size.value + 0.5);
		if (maxs.values[i] - m < 8)
			continue;
		if (m - mins.values[i] < 8)
			continue;
		...
		*(vec3_t*)v = *(vec3_t*)verts;
		...
		for (j = 0; j<numverts; j++, v += 3)
		{
			...
			if ((dist[j] > 0) != (dist[j+1] > 0))
			{
				...
				for (k = 0; k<3; k++)
					front[f].values[k] = back[b].values[k] = v[k] + frac*(v[3+k] - v[k]);
				f++;
				b++;
			}
		}

		SubdividePolygon(f, (float*)&front[0]);
		SubdividePolygon(b, (float*)&back[0]);
		return;
	}
	...
	{
		*(vec3_t*)poly->verts[i] = *(vec3_t*)verts;
		s = Vec_Dot(*(vec3_t*)verts, *(vec3_t*)warpface->texinfo->vecs[0]);
		t = Vec_Dot(*(vec3_t*)verts, *(vec3_t*)warpface->texinfo->vecs[1]);
		poly->verts[i][3] = s;
		poly->verts[i][4] = t;
	}
}
...
void GL_SubdivideSurface(msurface_t* fa)
{
	...
	for (i = 0; i<fa->polys->numverts; i++)
		verts[i] = *(vec3_t*)fa->polys->verts[i];

	SubdividePolygon(fa->polys->numverts, (float*)&verts[0]);
}
...
void R_UpdateWarpTextures(void)
{
	...
	warptess = 128.0/Clamp(floor(r_waterquality.value), 3.0, 64.0);
	...
	for (i = 0; i<cl.worldmodel->numtextures; i++)
	{
		...
		glCopyTexSubImage2D(GL_TEXTURE_2D, 0, 0, 0, 0, 0+vid.height-gl_warpimagesize, gl_warpimagesize, gl_warpimagesize);
	...
	if (gl_warpimagesize + sb_lines > vid.height)
		Sbar_Changed();
