...
cvar_t gl_vertexsnap = { "gl_vertexsnap", "1", CVAR_ARCHIVE };
...
void GLSLGamma_GammaCorrect(void)
{
	...
	GL_Uniform1fFunc(contrastLoc, Min(2.0, Max(1.0, vid_contrast.value)));
	...
	smax = vid.width/(float)r_gamma_texture_width;
	tmax = vid.height/(float)r_gamma_texture_height;
...
bool R_CullBox(vec3_t emins, vec3_t emaxs)
{
	...
	for (i = 0; i < 4; i++)
	{
		...
		default:
		case 0:
			if (p->normal.x*emaxs.x + p->normal.y*emaxs.y + p->normal.z*emaxs.z < p->dist)
				return true;
			break;
		case 1:
			if (p->normal.x*emins.x + p->normal.y*emaxs.y + p->normal.z*emaxs.z < p->dist)
				return true;
			break;
		case 2:
			if (p->normal.x*emaxs.x + p->normal.y*emins.y + p->normal.z*emaxs.z < p->dist)
				return true;
			break;
		case 3:
			if (p->normal.x*emins.x + p->normal.y*emins.y + p->normal.z*emaxs.z < p->dist)
				return true;
			break;
		case 4:
			if (p->normal.x*emaxs.x + p->normal.y*emaxs.y + p->normal.z*emins.z < p->dist)
				return true;
			break;
		case 5:
			if (p->normal.x*emins.x + p->normal.y*emaxs.y + p->normal.z*emins.z < p->dist)
				return true;
			break;
		case 6:
			if (p->normal.x*emaxs.x + p->normal.y*emins.y + p->normal.z*emins.z < p->dist)
				return true;
			break;
		case 7:
			if (p->normal.x*emins.x + p->normal.y*emins.y + p->normal.z*emins.z < p->dist)
				return true;
			break;
		}
	}
...
bool R_CullModelForEntity(entity_t* e)
{
	...
	if (e->angles.x || e->angles.z) //pitch or roll
	{
		mins = Vec_Add(e->origin, e->model->rmins);
		maxs = Vec_Add(e->origin, e->model->rmaxs);
	}
	else if (e->angles.y) //yaw
	{
		mins = Vec_Add(e->origin, e->model->ymins);
		maxs = Vec_Add(e->origin, e->model->ymaxs);
	}
	else //no rotation
	{
		mins = Vec_Add(e->origin, e->model->mins);
		maxs = Vec_Add(e->origin, e->model->maxs);
	}
...
void R_RotateForEntity(vec3_t origin, vec3_t angles)
{
	...
	glRotatef(angles.y, 0, 0, 1);
	glRotatef(-angles.x, 0, 1, 0);
	glRotatef(angles.z, 1, 0, 0);
}
...
void TurnVector(vec3_t* out, const vec3_t forward, const vec3_t side, float angle)
{
	...
	out->x = scale_forward*forward.x + scale_side*side.x;
	out->y = scale_forward*forward.y + scale_side*side.y;
	out->z = scale_forward*forward.z + scale_side*side.z;
}
...
void R_SetFrustum(float fovx, float fovy)
{
	...
	for (i = 0; i<4; i++)
	{
		...
		frustum[i].dist = Vec_Dot(r_origin, frustum[i].normal); //FIXME: shouldn't this always be zero?
...
void R_SetupGL(void)
{
	...
	glLoadIdentity();
	scale = Max((int)r_scale.value, 1); // ericw -- see R_ScaleView
	glViewport(0 + r_refdef.vrect.x,
		0 + vid.height - r_refdef.vrect.y - r_refdef.vrect.height,
		Max(r_refdef.vrect.width / scale, 4),
		Max(r_refdef.vrect.height / scale, 4));
	//johnfitz
...
// glCullFace(GL_BACK); //johnfitz -- glquake used CCW with backwards culling -- let's do it right
	...
	glRotatef(-r_refdef.viewangles.z, 1, 0, 0);
	glRotatef(-r_refdef.viewangles.x, 0, 1, 0);
	glRotatef(-r_refdef.viewangles.y, 0, 0, 1);
...
// build the transformation matrix for the given view angles
	...
	Vec_AngleVectors(r_refdef.viewangles, &vpn, &vright, &vup);

	// current viewleaf
	r_oldviewleaf = r_viewleaf;
	r_viewleaf = Mod_PointInLeaf(*(vec3_t*)r_origin.values, cl.worldmodel);
	...
	{
		int contents = Mod_PointInLeaf(*(vec3_t*)r_origin.values, cl.worldmodel)->contents;
...
void R_DrawEntity(entity_t* entity, bool alphapass)
{
	...
	//johnfitz -- chasecam
	if (currententity == &cl_entities[cl.entityindex])
		currententity->angles.x *= 0.3;
	//johnfitz
...
void R_DrawEntitiesOnList(bool alphapass) //johnfitz -- added parameter
{
	...
	{
		R_DrawEntity(cl_visedicts[i], alphapass);
	}
	for (int i = 0; i < cl_nummodeldraw; i++)
	{
		R_DrawEntity(cl_modeldraw+i, alphapass);
	}
}
...
{
	edict_t* sv_player = host_client->edict;
	vec3_t mins, maxs;
	edict_t* ed;
	int i;
	...
	for (i = 0, ed = NEXT_EDICT(sv.edicts); i<sv.num_edicts; i++, ed = NEXT_EDICT(ed))
	{
		...
		if (ed->v.mins.x == ed->v.maxs.x && ed->v.mins.y == ed->v.maxs.y && ed->v.mins.z == ed->v.maxs.z)
		{
			//point entity
		...
		{
			//box entity
			mins = Vec_Add(ed->v.mins, ed->v.origin);
			maxs = Vec_Add(ed->v.maxs, ed->v.origin);
...
void R_ShowTris(void)
{
	...
	if (r_drawentities.value)
	{
		for (i = 0; i<cl_numvisedicts; i++)
		{
			...
			if (currententity == &cl_entities[cl.entityindex]) // chasecam
				currententity->angles.x *= 0.3;
...
void R_ScaleView(void)
{
	...
	scale = Max((int)r_scale.value, 1);

	srcx = 0 + r_refdef.vrect.x;
	srcy = 0 + vid.height - r_refdef.vrect.y - r_refdef.vrect.height;
	srcw = Max(r_refdef.vrect.width / scale, 4);
	srch = Max(r_refdef.vrect.height / scale, 4);
...
void R_RenderView(void)
{
	...
	if (r_stereo.value)
	{
		float eyesep = Clamp(r_stereo.value, -8.0f, 8.0f);
		float fdepth = Clamp(r_stereodepth.value, 32.0f, 1024.0f);

		Vec_AngleVectors(r_refdef.viewangles, &vpn, &vright, &vup);

		//render left eye (red)
		glColorMask(1, 0, 0, 1);
		r_refdef.vieworg = Vec_MulAdd(r_refdef.vieworg, -0.5f * eyesep, vright);
		...
		glColorMask(0, 1, 1, 1);
		r_refdef.vieworg = Vec_MulAdd(r_refdef.vieworg, 1.0f * eyesep, vright);
	...
	if (r_pos.value)
		...
		(int)cl_entities[cl.entityindex].origin.x,
			(int)cl_entities[cl.entityindex].origin.y,
			(int)cl_entities[cl.entityindex].origin.z,
