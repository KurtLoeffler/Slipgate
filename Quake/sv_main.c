...
void SV_Protocol_f(void)
{
	...
	case 2:
		i = atoi(Cmd_Argv(1));
		if (i != PROTOCOL_NETQUAKE && i != PROTOCOL_FITZQUAKE && i != PROTOCOL_RMQ && i != PROTOCOL_SLIPGATE)
			Con_Printf("sv_protocol must be %i or %i or %i or %i\n", PROTOCOL_NETQUAKE, PROTOCOL_FITZQUAKE, PROTOCOL_RMQ, PROTOCOL_SLIPGATE);
		else
		{
			sv_protocol = i;
			if (sv.active)
...
void SV_Init(void)
{
	...
	case PROTOCOL_RMQ:
		p = "RMQ";
		break;
	case PROTOCOL_SLIPGATE:
		p = "Slipgate";
		break;
	default:
		Sys_Error("Bad protocol version request %i. Accepted values: %i, %i, %i, %i.",
			sv_protocol, PROTOCOL_NETQUAKE, PROTOCOL_FITZQUAKE, PROTOCOL_RMQ, PROTOCOL_SLIPGATE);
...
void SV_StartParticle(vec3_t org, vec3_t dir, int color, int count)
{
	...
	MSG_WriteCoord(&sv.datagram, org.x, sv.protocolflags);
	MSG_WriteCoord(&sv.datagram, org.y, sv.protocolflags);
	MSG_WriteCoord(&sv.datagram, org.z, sv.protocolflags);
	for (i = 0; i<3; i++)
	{
		v = dir.values[i]*16;
		if (v > 127)
...
// directed messages go only to the entity the are targeted on
	...
	for (i = 0; i < 3; i++)
		MSG_WriteCoord(&sv.datagram, entity->v.origin.values[i]+0.5*(entity->v.mins.values[i]+entity->v.maxs.values[i]), sv.protocolflags);
}
...
void SV_AddToFatPVS(vec3_t org, mnode_t* node, qmodel_t* worldmodel) //johnfitz -- added worldmodel as a parameter
{
	...
	while (1)
	{
		...
		plane = node->plane;
		d = Vec_Dot(org, plane->normal) - plane->dist;
		if (d > 8)
...
bool SV_VisibleToClient(edict_t* client, edict_t* test, qmodel_t* worldmodel)
{
	...
	org = Vec_Add(client->v.origin, client->v.view_ofs);
...
char writebagpath[256];
void WriteBag(edict_t* ent, sizebuf_t* msg)
{
	int namecount;
	...
	if (namecount)
	{
		...
		{
			BagValueType valuetype;
			float numbervalue;
			bool boolvalue;
			char* stringvalue;
			QLua_GetBagItem(ent, writebagpath, name, &valuetype, &numbervalue, &boolvalue, &stringvalue);

			if (valuetype == bagv_none)
			{
				Host_Error("valuetype is bagv_none\n");
				break;
			}

			MSG_WriteString(msg, name);
			MSG_WriteChar(msg, valuetype);
			if (valuetype == bagv_number)
			{
				MSG_WriteFloat(msg, numbervalue);
			}
			else if (valuetype == bagv_boolean)
			{
				MSG_WriteByte(msg, boolvalue);
			}
			else if (valuetype == bagv_string)
			{
				MSG_WriteString(msg, stringvalue);
			}
			else if (valuetype == bagv_table)
			{
				int oldlen = strlen(writebagpath);
				strcat(writebagpath, ".");
				strcat(writebagpath, name);
				WriteBag(ent, msg);
				writebagpath[oldlen] = 0;
			}
...
void SV_WriteEntitiesToClient(edict_t* clent, sizebuf_t* msg)
{
	...
	org = Vec_Add(clent->v.origin, clent->v.view_ofs);
	...
	for (e = 1; e<sv.num_edicts; e++, ent = NEXT_EDICT(ent))
	{
		...
		for (i = 0; i<3; i++)
		{
			miss = ent->v.origin.values[i] - ent->baseline.origin.values[i];
		...
		if (bits & U_ORIGIN1)
			MSG_WriteCoord(msg, ent->v.origin.x, sv.protocolflags);
		if (bits & U_ANGLE1)
			MSG_WriteAngle(msg, ent->v.angles.x, sv.protocolflags);
		if (bits & U_ORIGIN2)
			MSG_WriteCoord(msg, ent->v.origin.y, sv.protocolflags);
		if (bits & U_ANGLE2)
			MSG_WriteAngle(msg, ent->v.angles.y, sv.protocolflags);
		if (bits & U_ORIGIN3)
			MSG_WriteCoord(msg, ent->v.origin.z, sv.protocolflags);
		if (bits & U_ANGLE3)
			MSG_WriteAngle(msg, ent->v.angles.z, sv.protocolflags);
		...
		if (sv.protocol == PROTOCOL_SLIPGATE)
		{
			byte sbits = 0;

			if (!Vec_Equal(ent->v.mins, Vec_Zero()) || !Vec_Equal(ent->v.maxs, Vec_Zero()))
			{
				sbits |= SGU_MINMAXS;
			}

			MSG_WriteChar(msg, sbits);
			MSG_WriteLong(msg, ent->id);

			if (sbits & SGU_MINMAXS)
			{
				MSG_WriteCoord(msg, ent->v.mins.x, sv.protocolflags);
				MSG_WriteCoord(msg, ent->v.mins.y, sv.protocolflags);
				MSG_WriteCoord(msg, ent->v.mins.z, sv.protocolflags);
				MSG_WriteCoord(msg, ent->v.maxs.x, sv.protocolflags);
				MSG_WriteCoord(msg, ent->v.maxs.y, sv.protocolflags);
				MSG_WriteCoord(msg, ent->v.maxs.z, sv.protocolflags);
			}

			MSG_WriteChar(msg, ent->v.solid);

			if (qlua.state)
			{
				writebagpath[0] = 0;
				strcat(writebagpath, "bag");
				WriteBag(ent, msg);
			}
		}
		//johnfitz
...
void SV_WriteClientdataToMessage(edict_t* ent, sizebuf_t* msg)
{
	...
	if (ent->v.dmg_take || ent->v.dmg_save)
	{
		...
		for (i = 0; i<3; i++)
			MSG_WriteCoord(msg, other->v.origin.values[i] + 0.5*(other->v.mins.values[i] + other->v.maxs.values[i]), sv.protocolflags);
	...
	//
		//SV_SetIdealPitch(ent); // how much to look up / down ideally
	...
	if (ent->v.fixangle)
	{
		...
		for (i = 0; i < 3; i++)
			MSG_WriteAngle(msg, ent->v.angles.values[i], sv.protocolflags);
		ent->v.fixangle = 0;
	}

	bits = 0;

	if (ent->v.view_ofs.z != DEFAULT_VIEWHEIGHT)
		bits |= SU_VIEWHEIGHT;
	...
	for (i = 0; i<3; i++)
	{
		if (ent->v.punchangle.values[i])
			bits |= (SU_PUNCH1<<i);
		if (ent->v.velocity.values[i])
			bits |= (SU_VELOCITY1<<i);
	}
...
// send the data
	...
	if (bits & SU_VIEWHEIGHT)
		MSG_WriteChar(msg, ent->v.view_ofs.z);
	...
	{
		if (bits & (SU_PUNCH1<<i))
			MSG_WriteChar(msg, ent->v.punchangle.values[i]);
		if (bits & (SU_VELOCITY1<<i))
			MSG_WriteChar(msg, ent->v.velocity.values[i]/16);
	}
...
=======================
*/
byte sendclientdatagrambuf[MAX_DATAGRAM];
bool SV_SendClientDatagram(client_t* client)
{
	//byte buf[MAX_DATAGRAM];
	sizebuf_t msg;

	msg.data = sendclientdatagrambuf;
	msg.maxsize = sizeof(sendclientdatagrambuf);
	msg.cursize = 0;
...
void SV_UpdateToReliableMessages(void)
{
	...
	if (qlua.state)
	{
		for (j = 0, client = svs.clients; j<svs.maxclients; j++, client++)
		{
			...
			for (int i = 0; i < qlua.rpccount; i++)
			{
				rpc_t* rpc = &qlua.rpcs[i];
				MSG_WriteByte(&client->message, svc_rpc);
				MSG_WriteShort(&client->message, rpc->size);
				MSG_WriteString(&client->message, rpc->funcname);
				for (int k = 0; k < rpc->size; k++)
				{
					MSG_WriteByte(&client->message, rpc->data[k]);
				}
			}
		}
...
void SV_CreateBaseline(void)
{
	...
	//
		...
		for (i = 0; i<3; i++)
		{
			MSG_WriteCoord(&sv.signon, svent->baseline.origin.values[i], sv.protocolflags);
			MSG_WriteAngle(&sv.signon, svent->baseline.angles.values[i], sv.protocolflags);
		}
...
//
	...
	sv.max_edicts = Clamp((int)max_edicts.value, MIN_EDICTS, MAX_EDICTS); //johnfitz -- max_edicts cvar
