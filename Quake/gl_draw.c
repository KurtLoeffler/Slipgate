...
/*
================
Draw_CachePicFromTGA
================
*/
qpic_t* Draw_CachePicFromImage(const char* path, cachepic_t* pic)
{
	char image_path[MAX_QPATH];
	COM_StripExtension(path, image_path, sizeof(image_path));

	int width, height;
	byte* bytes = Image_LoadImage(image_path, &width, &height);
	if (!bytes)
	{
	...
	glpic_t gl;
	gl.gltexture = TexMgr_LoadImage(NULL, image_path, width, height, SRC_RGBA, bytes, image_path, 0, TEXPREF_ALPHA | TEXPREF_PAD | TEXPREF_NOPICMIP); //johnfitz -- TexMgr
	gl.sl = 0;
	gl.sh = (float)width/(float)TexMgr_PadConditional(width); //johnfitz
	gl.tl = 0;
	gl.th = (float)height/(float)TexMgr_PadConditional(height); //johnfitz
...
qpic_t* Draw_CachePic(const char* path)
{
	...
	if (!dat)
	{
		return Draw_CachePicFromImage(path, pic);
	}
...
void GL_SetCanvas(canvastype newcanvas)
{
	...
	case CANVAS_MENU:
		...
		s = Clamp(scr_menuscale.value, 1.0, s);
		// ericw -- doubled width to 640 to accommodate long keybindings
		glOrtho(0, 640, 200, 0, -99999, 99999);
		glViewport(0 + (vid.width - 320*s) / 2, 0 + (vid.height - 200*s) / 2, 640*s, 200*s);
		break;
	case CANVAS_SBAR:
		s = Clamp(scr_sbarscale.value, 1.0, (float)vid.width / 320.0);
		if (cl.gametype == GAME_DEATHMATCH)
		{
			glOrtho(0, vid.width / s, 48, 0, -99999, 99999);
			glViewport(0, 0, vid.width, 48*s);
		}
		else
		{
			glOrtho(0, 320, 48, 0, -99999, 99999);
			glViewport(0 + (vid.width - 320*s) / 2, 0, 320*s, 48*s);
		}
		break;
	case CANVAS_WARPIMAGE:
		glOrtho(0, 128, 0, 128, -99999, 99999);
		glViewport(0, 0+vid.height-gl_warpimagesize, gl_warpimagesize, gl_warpimagesize);
		break;
	case CANVAS_CROSSHAIR: //0,0 is center of viewport
		s = Clamp(scr_crosshairscale.value, 1.0, 10.0);
	...
	case CANVAS_BOTTOMLEFT: //used by devstats
		...
		glViewport(0, 0, 320*s, 200*s);
		break;
	case CANVAS_BOTTOMRIGHT: //used by fps/clock
		s = (float)vid.width/vid.conwidth; //use console scale
		glOrtho(0, 320, 200, 0, -99999, 99999);
		glViewport(0+vid.width-320*s, 0, 320*s, 200*s);
		break;
	case CANVAS_TOPRIGHT: //used by disc
		s = 1;
		glOrtho(0, 320, 200, 0, -99999, 99999);
		glViewport(0+vid.width-320*s, 0+vid.height-200*s, 320*s, 200*s);
		break;
	default:
