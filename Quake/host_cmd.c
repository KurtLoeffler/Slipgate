...
void ExtraMaps_Init(void)
{
	...
	q_snprintf(ignorepakdir, sizeof(ignorepakdir), "/%s/", defaultgamename);
...
void Host_SetPos_f(void)
{
	...
	if (Cmd_Argc() != 7 && Cmd_Argc() != 4)
	{
		...
		SV_ClientPrintf("   %i %i %i %i %i %i\n",
			(int)sv_player->v.origin.x,
			(int)sv_player->v.origin.y,
			(int)sv_player->v.origin.z,
			(int)sv_player->v.v_angle.x,
			(int)sv_player->v.v_angle.y,
			(int)sv_player->v.v_angle.z);
		return;
	}
	...
	sv_player->v.velocity.x = 0;
	sv_player->v.velocity.y = 0;
	sv_player->v.velocity.z = 0;

	sv_player->v.origin.x = atof(Cmd_Argv(1));
	sv_player->v.origin.y = atof(Cmd_Argv(2));
	sv_player->v.origin.z = atof(Cmd_Argv(3));

	if (Cmd_Argc() == 7)
	{
		sv_player->v.angles.x = atof(Cmd_Argv(4));
		sv_player->v.angles.y = atof(Cmd_Argv(5));
		sv_player->v.angles.z = atof(Cmd_Argv(6));
...
void Host_Spawn_f(void)
{
	...
	for (i = 0; i < 2; i++)
		MSG_WriteAngle(&host_client->message, ent->v.angles.values[i], sv.protocolflags);
