...
void Sky_ProjectPoly(int nump, vec3_t* vecs)
{
	...
	v = Vec_Zero();
	for (i = 0, vp = (float*)vecs; i<nump; i++, vp += 3)
	{
		v = Vec_Add(*(vec3_t*)vp, v);
	}
	av.x = fabs(v.x);
	av.y = fabs(v.y);
	av.z = fabs(v.z);
	...
	for (i = 0; i<nump; i++, vecs += 3)
	{
		...
		j = floato_st[axis][0];
		if (j < 0)
			s = -vecs->values[-j -1] / dv;
		else
			s = vecs->values[j-1] / dv;
		j = floato_st[axis][1];
		if (j < 0)
			t = -vecs->values[-j -1] / dv;
		else
			t = vecs->values[j-1] / dv;
...
void Sky_ClipPoly(int nump, vec3_t* vecs, int stage)
{
	...
	front = back = false;
	norm = skyclip[stage].values;
	for (i = 0, v = (float*)vecs; i<nump; i++, v += 3)
	{
		d = Vec_Dot(*(vec3_t*)v, *(vec3_t*)norm);
		if (d > ON_EPSILON)
		{
			front = true;
			sides[i] = SIDE_FRONT;
	...
	*(vecs+(i)) = *(vec3_t*)vecs;
	...
	for (i = 0, v = (float*)vecs; i<nump; i++, v += 3)
	{
		...
		{
		case SIDE_FRONT:
			newv[0][newc[0]] = *(vec3_t*)v;
			newc[0]++;
			break;
		case SIDE_BACK:
			newv[1][newc[1]] = *(vec3_t*)v;
			newc[1]++;
			break;
		case SIDE_ON:
			newv[0][newc[0]] = *(vec3_t*)v;
		...
		{
			e = v[j] + d*(v[j+3] - v[j]);
			newv[0][newc[0]].values[j] = e;
			newv[1][newc[1]].values[j] = e;
		}
		newc[0]++;
		newc[1]++;
	}
...
void Sky_ProcessPoly(glpoly_t* p)
{
	...
	if (!r_fastsky.value)
	{
		for (i = 0; i<p->numverts; i++)
			verts[i] = Vec_Sub(*(vec3_t*)p->verts[i], r_origin);
...
void Sky_ProcessEntities(void)
{
	...
	for (i = 0; i<cl_numvisedicts; i++)
	{
		...
		for (j = 0; j<e->model->nummodelsurfaces; j++, s++)
		{
			...
			{
				dot = Vec_Dot(modelorg, s->plane->normal) - s->plane->dist;
					...
					(!(s->flags & SURF_PLANEBACK) && (dot > BACKFACE_EPSILON)))
				{
					...
					for (k = 0; k<p->numverts; k++)
					{
						if (rotated)
						{
							p->verts[k][0] = e->origin.x + s->polys->verts[k][0] * forward.x
								- s->polys->verts[k][1] * right.x
								+ s->polys->verts[k][2] * up.x;
							p->verts[k][1] = e->origin.y + s->polys->verts[k][0] * forward.y
								- s->polys->verts[k][1] * right.y
								+ s->polys->verts[k][2] * up.y;
							p->verts[k][2] = e->origin.z + s->polys->verts[k][0] * forward.z
								- s->polys->verts[k][1] * right.z
								+ s->polys->verts[k][2] * up.z;
						}
						else
							*(vec3_t*)p->verts[k] = Vec_Add(*(vec3_t*)s->polys->verts[k], e->origin);
					}
					Sky_ProcessPoly(p);
					Hunk_FreeToLowMark(mark);
				}
...
void Sky_EmitSkyBoxVertex(float s, float t, int axis)
{
	...
	b.x = s * gl_farclip.value / sqrt(3.0);
	b.y = t * gl_farclip.value / sqrt(3.0);
	b.z = gl_farclip.value / sqrt(3.0);

	for (j = 0; j<3; j++)
	{
		k = st_to_vec[axis][j];
		if (k < 0)
			v.values[j] = -b.values[-k - 1];
		else
			v.values[j] = b.values[k - 1];
		v.values[j] += r_origin.values[j];
	}
...
void Sky_DrawSkyBox(void)
{
	...
	for (i = 0; i<6; i++)
	{
		...
		if (Fog_GetDensity() > 0 && skyfog > 0)
		{
			...
			glColor4f(c[0], c[1], c[2], Clamp(skyfog, 0.0, 1.0));
...
void Sky_GetTexCoord(vec3_t v, float speed, float* s, float* t)
{
	...
	*s = (scroll + dir.x * length) * (1.0/128);
	*t = (scroll + dir.y * length) * (1.0/128);
}
...
void Sky_DrawFaceQuad(glpoly_t* p)
{
	...
	if (gl_mtexable && r_skyalpha.value >= 1.0)
	{
		...
		{
			Sky_GetTexCoord(*(vec3_t*)v, 8, &s, &t);
			GL_MTexCoord2fFunc(GL_TEXTURE0_ARB, s, t);
			Sky_GetTexCoord(*(vec3_t*)v, 16, &s, &t);
...
void Sky_DrawFace(int axis)
{
	...
	vup = Vec_Sub(verts[2], verts[3]);
	vright = Vec_Sub(verts[2], verts[1]);
	...
	for (i = 0; i<di; i++)
	{
		for (j = 0; j<dj; j++)
		{
			...
			temp = Vec_Scale(vright, qi*i);
			temp2 = Vec_Scale(vup, qj*j);
			temp = Vec_Add(temp, temp2);
			*(vec3_t*)p->verts[0] = Vec_Add(verts[0], temp);

			temp = Vec_Scale(vup, qj);
			*(vec3_t*)p->verts[1] = Vec_Add(*(vec3_t*)p->verts[0], temp);

			temp = Vec_Scale(vright, qi);
			*(vec3_t*)p->verts[2] = Vec_Add(*(vec3_t*)p->verts[1], temp);

			*(vec3_t*)p->verts[3] = Vec_Add(*(vec3_t*)p->verts[0], temp);
