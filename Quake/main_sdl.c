...
{
	int gamefilehandle;
	int length = Sys_FileOpenRead("game.txt", &gamefilehandle);
	if (length < 0 || length >= sizeof(defaultgamename))
	{
		Con_Printf("Could not load game.txt\n");
		return;
	}

	char readbuffer[MAX_OSPATH];
	Sys_FileRead(gamefilehandle, (void*)&readbuffer, length);
	readbuffer[length] = 0;
	char* pos = readbuffer;
	while (*pos)
	{
	...
	strcpy(defaultgamename, readbuffer);
	Sys_FileClose(gamefilehandle);
}
...
int main(int argc, char* argv[])
{
	...
	Sys_Printf("Slipgate " SLIPGATE_VER_STRING " (c) Kurt Loeffler\n");
