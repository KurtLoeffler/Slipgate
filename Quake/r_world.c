...
extern cvar_t gl_fullbrights, r_drawflat, gl_overbright, gl_vertexsnap, r_oldskyleaf, r_showtris; //johnfitz
...
#define positionAttrIndex 0
#define texCoordsAttrIndex 1
static GLuint r_brush_program;

// uniforms used in vert shader
static GLuint screenquantizationLoc;
static GLuint usescreenquantizationLoc;

// uniforms used in frag shader
static GLuint texLoc;
static GLuint lightmapTexLoc;
static GLuint splatmapTexLoc;
static GLuint fullbrightTexLoc;
static GLuint useFullbrightTexLoc;
static GLuint uselightmapTexLoc;
static GLuint overbrightLoc;
static GLuint useAlphaTestLoc;

void R_CreateBrushShader(void)
{
	const glsl_attrib_binding_t bindings[] = {
		{ "Position", positionAttrIndex },
	...
	const GLchar* vertSource = \
		...
		"uniform bool UseScreenQuantization;\n"
	...
	const GLchar* fragSource = \
		...
		"	if (alpha < 0.666) discard;"
		"	result = clamp(result, 0.0, 1.0);\n"
		"	if (UseLightmapTex) {\n"
		"		result *= texture2D(LightmapTex, gl_TexCoord[1].xy)*Overbright;\n"
		"		result *= texture2D(SplatmapTex, gl_TexCoord[1].xy);\n"
		"	}"
		"	if (UseFullbrightTex)\n"
		...
		"	result.a = 1.0;\n"
		//"	result = mix(result, vec4(1.0, 1.0, 1.0, 1.0), 0.9);\n"
		"	gl_FragColor = result;\n"
		"}\n";
	...
	r_brush_program = GL_CreateProgram(vertSource, fragSource, sizeof(bindings)/sizeof(bindings[0])*0, bindings);
	Con_Printf("Brush shader program ID is %d\n", r_brush_program);

	if (r_brush_program != 0)
	{
		// get uniform locations
		screenquantizationLoc = GL_GetUniformLocation(&r_brush_program, "ScreenQuantization");
		usescreenquantizationLoc = GL_GetUniformLocation(&r_brush_program, "UseScreenQuantization");
		texLoc = GL_GetUniformLocation(&r_brush_program, "Tex");
		lightmapTexLoc = GL_GetUniformLocation(&r_brush_program, "LightmapTex");
		splatmapTexLoc = GL_GetUniformLocation(&r_brush_program, "SplatmapTex");
		fullbrightTexLoc = GL_GetUniformLocation(&r_brush_program, "FullbrightTex");
		useFullbrightTexLoc = GL_GetUniformLocation(&r_brush_program, "UseFullbrightTex");
		uselightmapTexLoc = GL_GetUniformLocation(&r_brush_program, "UseLightmapTex");
		overbrightLoc = GL_GetUniformLocation(&r_brush_program, "Overbright");
		//useAlphaTestLoc = GL_GetUniformLocation(&r_brush_program, "UseAlphaTest");
	}
}
...
bool R_BackFaceCull(msurface_t* surf)
{
	...
	{
	case PLANE_X:
		dot = r_refdef.vieworg.x - surf->plane->dist;
		break;
	case PLANE_Y:
		dot = r_refdef.vieworg.y - surf->plane->dist;
		break;
	case PLANE_Z:
		dot = r_refdef.vieworg.z - surf->plane->dist;
		break;
	default:
		dot = Vec_Dot(r_refdef.vieworg, surf->plane->normal) - surf->plane->dist;
		break;
	}
...
void R_CullSurfaces(void)
{
	...
	for (i = 0; i<cl.worldmodel->numtextures; i++)
	{
		...
		{
			if (R_CullBox(*(vec3_t*)s->mins, *(vec3_t*)s->maxs) || R_BackFaceCull(s))
				s->culled = true;
			else
			{
				s->culled = false;
...
void R_DrawTextureChains_Water(qmodel_t* model, entity_t* ent, texchain_t chain)
{
	...
	if (useshader)
	{
		...
		GL_Uniform1iFunc(uselightmapTexLoc, 0);
	}
...
void R_DrawTextureChains_Multitexture_VBO(qmodel_t* model, entity_t* ent, texchain_t chain)
{
	...
	if (useshader)
	{
		GL_UseProgramFunc(r_brush_program);
		GL_Uniform1iFunc(texLoc, 0);
		GL_Uniform1iFunc(lightmapTexLoc, 1);
		GL_Uniform1iFunc(fullbrightTexLoc, 2);
		GL_Uniform1iFunc(splatmapTexLoc, 3);

		GL_Uniform1fFunc(overbrightLoc, gl_overbright.value ? 4.0f : 1.0f);

		GL_Uniform1iFunc(uselightmapTexLoc, 1);

		GL_Uniform1iFunc(usescreenquantizationLoc, gl_vertexsnap.value > 0 ? 1 : 0);
		if (gl_vertexsnap.value > 0)
		{
			GL_Uniform3fFunc(screenquantizationLoc, r_refdef.vrect.width/r_scale.value/gl_vertexsnap.value/2, r_refdef.vrect.height/r_scale.value/gl_vertexsnap.value/2, 0);
		}

		//GL_EnableVertexAttribArrayFunc(positionAttrIndex);
		//GL_EnableVertexAttribArrayFunc(texCoordsAttrIndex);
		
		//GL_VertexAttribPointerFunc(positionAttrIndex, 4, GL_FLOAT, GL_FALSE, sizeof(meshxyz_t), );
	...
	else
	{
		...
		glTexEnvf(GL_TEXTURE_ENV, GL_RGB_SCALE_EXT, gl_overbright.value ? 4.0f : 1.0f);
		glEnable(GL_TEXTURE_2D);
	...
	for (i = 0; i<model->numtextures; i++)
	{
		...
		if (useshader)
		{
			...
			{
				fullbright = R_TextureAnimation(t, ent != NULL ? ent->frame : 0)->fullbright;
			}
			
			GL_Uniform1iFunc(useFullbrightTexLoc, fullbright ? 1 : 0);
		...
		for (s = t->texturechains[chain]; s; s = s->texturechain)
			if (!s->culled)
			{
				...
				if (useshader)
				{
					...
					GL_Bind(splatmap_textures[s->lightmaptexturenum]);
				}
				else
				{
					GL_SelectTexture(GL_TEXTURE1_ARB);
	...
	{
		GL_UseProgramFunc(0);
		//GL_DisableVertexAttribArrayFunc(positionAttrIndex);
