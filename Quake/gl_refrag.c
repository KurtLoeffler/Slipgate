...
void R_CheckEfrags(void)
{
	...
	dev_peakstats.efrags = Max(cl.num_efrags, dev_peakstats.efrags);
}
...
void R_AddEfrags(entity_t* ent)
{
	...
	for (i = 0; i<3; i++)
	{
		r_emins.values[i] = ent->origin.values[i] + entmodel->mins.values[i];
		r_emaxs.values[i] = ent->origin.values[i] + entmodel->maxs.values[i];
	}
