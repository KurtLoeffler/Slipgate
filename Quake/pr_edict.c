...
edict_t* ED_Alloc(void)
{
	...
	for (i = svs.maxclients + 1; i < sv.num_edicts; i++)
	{
		...
		if (!e->id && (e->freetime < 2 || sv.time - e->freetime > 0.5))
		{
			ED_ClearEdict(e);
			ED_InitEdict(e);
			return e;
...
void ED_InitEdict(edict_t* e)
{
	e->id = pr_nextentityid;
	pr_nextentityid++;
}
...
void ED_Free(edict_t* ed)
{
	if (qlua.state)
	{
		int index = NUM_FOR_EDICT(ed);
		int popcount = 1;
		QLua_PushLuaFunction(qlua.state, &qlua.onfreeentityfunc);
		lua_pushnumber(qlua.state, index);
		QLua_CallFunction(qlua.state, 1, 0);
		lua_pop(qlua.state, popcount);//Pop function table.
	}
	...
	ed->v.skin = 0;
	ed->v.frame = 0;
	ed->v.origin = Vec_Zero();
	ed->v.angles = Vec_Zero();
	ed->v.nextthink = -1;
	ed->v.solid = 0;
...
void PR_LoadProgs(void)
{
	...
	for (i = 0; i < progs->numglobaldefs; i++)
	{
		...
		//if (pr_globaldefs[i].type & DEF_SAVEGLOBAL)
		//{
		//	Con_Printf("%s %i %i\n", PR_GetString(pr_globaldefs[i].s_name), pr_globaldefs[i].type & ~DEF_SAVEGLOBAL, pr_globaldefs[i].ofs);
		//}
