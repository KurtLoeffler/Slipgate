...
void Chase_UpdateForDrawing(void)
{
	...
	Vec_AngleVectors(cl.viewangles, &forward, &right, &up);

	// calc ideal camera location before checking for walls
	for (i = 0; i<3; i++)
		ideal.values[i] = cl.viewent.origin.values[i]
		- forward.values[i]*chase_back.value
		+ right.values[i]*chase_right.value;
	//+ up[i]*chase_up.value;
	ideal.z = cl.viewent.origin.z + chase_up.value;
	...
	// place camera
	r_refdef.vieworg = ideal;

	// find the spot the player is looking at
	temp = Vec_MulAdd(cl.viewent.origin, 4096, forward);
	TraceLine(cl.viewent.origin, temp, &crosshair);

	// calculate camera angles to look at the same spot
	temp = Vec_Sub(crosshair, r_refdef.vieworg);
	r_refdef.viewangles = Vec_Angles(temp);
