...
float V_CalcBob(void)
{
	...
	bob = sqrt(cl.velocity.x*cl.velocity.x + cl.velocity.y*cl.velocity.y) * cl_bob.value;
	//Con_Printf ("speed: %5.1f\n", Vec_Length(cl.velocity));
...
void V_ParseDamage(void)
{
	...
	blood = MSG_ReadByte();
	for (i = 0; i<3; i++)
		from.values[i] = MSG_ReadCoord(cl.protocolflags);
	...
	//
	ent = &cl_entities[cl.entityindex];

	from = Vec_Sub(from, ent->origin);
	Vec_Normalize(&from);

	Vec_AngleVectors(ent->angles, &forward, &right, &up);

	side = Vec_Dot(from, right);
	v_dmg_roll = count*side*v_kickroll.value;

	side = Vec_Dot(from, forward);
...
void V_BoundOffsets(void)
{
	...
	if (r_refdef.vieworg.x < ent->origin.x - 14)
		r_refdef.vieworg.x = ent->origin.x - 14;
	else if (r_refdef.vieworg.x > ent->origin.x + 14)
		r_refdef.vieworg.x = ent->origin.x + 14;
	if (r_refdef.vieworg.y < ent->origin.y - 14)
		r_refdef.vieworg.y = ent->origin.y - 14;
	else if (r_refdef.vieworg.y > ent->origin.y + 14)
		r_refdef.vieworg.y = ent->origin.y + 14;
	if (r_refdef.vieworg.z < ent->origin.z - 22)
		r_refdef.vieworg.z = ent->origin.z - 22;
	else if (r_refdef.vieworg.z > ent->origin.z + 30)
		r_refdef.vieworg.z = ent->origin.z + 30;
}
...
void V_CalcViewRoll(void)
{
	...
	side = V_CalcRoll(cl_entities[cl.entityindex].angles, cl.velocity);
...
// ent is the player model (visible when out of body)
	...
	if (cl.maxclients <= 1) //johnfitz -- moved cheat-protection here from V_RenderView
		for (i = 0; i<3; i++)
			r_refdef.vieworg.values[i] += scr_ofsx.value*forward.values[i] + scr_ofsy.value*right.values[i] + scr_ofsz.value*up.values[i];
	...
	for (i = 0; i<3; i++)
		view->origin.values[i] += forward.values[i]*bob*0.4;
	view->origin.z += bob;
	...
	if (v_gunkick.value == 1) //original quake kick
		r_refdef.viewangles = Vec_Add(r_refdef.viewangles, cl.punchangle);
	if (v_gunkick.value == 2) //lerped kick
	{
		for (i = 0; i<3; i++)
			if (punch.values[i] != v_punchangles[0].values[i])
			{
				//speed determined by how far we need to lerp in 1/10th of a second
				delta = (v_punchangles[0].values[i]-v_punchangles[1].values[i]) * host_frametime * 10;

				if (delta > 0)
					punch.values[i] = Min(punch.values[i]+delta, v_punchangles[0].values[i]);
				else if (delta < 0)
					punch.values[i] = Max(punch.values[i]+delta, v_punchangles[0].values[i]);
			}

		r_refdef.viewangles = Vec_Add(r_refdef.viewangles, punch);
	}
	//johnfitz

	// smooth out stair step ups
	if (!noclip_anglehack && cl.onground && ent->origin.z - oldz > 0) //johnfitz -- added exception for noclip
	//FIXME: noclip_anglehack is set on the server, so in a nonlocal game this won't work.
	{
		...
		oldz += steptime * 80;
		if (oldz > ent->origin.z)
			oldz = ent->origin.z;
		if (ent->origin.z - oldz > 12)
			oldz = ent->origin.z - 12;
		r_refdef.vieworg.z += oldz - ent->origin.z;
		view->origin.z += oldz - ent->origin.z;
	}
	else
		oldz = ent->origin.z;
