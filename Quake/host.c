...
cvar_t max_edicts = { "max_edicts", "32000", CVAR_NONE }; //johnfitz //ericw -- changed from 2048 to 8192, removed CVAR_ARCHIVE
...
void Host_Version_f(void)
{
	...
	Con_Printf("Slipgate Version " SLIPGATE_VER_STRING "\n");
...
#if 0
		f = fopen(va("%s/fitzquake.rc", defaultgamename), "w"); //always save in id1
		if (!f)
		{
...
bool Host_FilterTime(float time)
{
	...
	maxfps = Clamp(host_maxfps.value, 10.0, 1000.0);
	...
	//johnfitz
		...
		host_frametime = Clamp(host_frametime, 0.001, 0.1); //johnfitz -- use CLAMP
...
// get new key events
	...
	if (cls.signon == SIGNONS)
	{
		...
		S_Update(Vec_Zero(), Vec_Zero(), Vec_Zero(), Vec_Zero());
