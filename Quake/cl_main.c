...
entity_t cl_modeldraw[MAX_MODELDRAW];
int cl_nummodeldraw;
...
void CL_ClearState(void)
{
	...
	cl_max_edicts = Clamp((int)max_edicts.value, MIN_EDICTS, MAX_EDICTS);
...
void CL_PrintEntities_f(void)
{
	...
	for (i = 0, ent = cl_entities; i<cl.num_entities; i++, ent++)
	{
		...
		Con_Printf("%s:%2i  (%5.1f,%5.1f,%5.1f) [%5.1f %5.1f %5.1f]\n"
			, ent->model->name, ent->frame, ent->origin.x, ent->origin.y, ent->origin.z, ent->angles.x, ent->angles.y, ent->angles.z);
	}
}
	...
	if (key)
	{
		...
		for (i = 0; i<MAX_DLIGHTS; i++, dl++)
		{
			if (dl->key == key)
			{
				...
				dl->key = key;
				dl->color.x = dl->color.y = dl->color.z = 1; //johnfitz -- lit support via lordhavoc
				return dl;
			}
		}
	}
...
void CL_RelinkEntities(void)
{
	...
	cl.deltatime = cl.time-cl.oldtime;

	cl_numvisedicts = 0;

	// interpolate player info
	cl.velocity = Vec_Lerp(cl.mvelocity[1], cl.mvelocity[0], frac);

	if (cls.demoplayback)
	{
		// interpolate the angles
		cl.viewangles = Vec_LerpAngles(cl.mviewangles[1], cl.mviewangles[0], frac);
	}
	...
	for (i = 1, ent = cl_entities+1; i<cl.num_entities; i++, ent++)
	{
		...
		{ // the entity was not updated in the last message
			...
			if (cl.deltatime)
			{
				ent->velocity = Vec_Scale(Vec_Sub(ent->origin, oldorg), 1.0/cl.deltatime);
			}
		}
		else
		{ // if the delta is large, assume a teleport and don't lerp
			...
			for (j = 0; j<3; j++)
			{
				float delta = ent->msg_origins[0].values[j] - ent->msg_origins[1].values[j];
				//delta.values[j] = ent->msg_origins[0].values[j] - ent->msg_origins[1].values[j];
			...
			{
				Con_Printf("%s %i\n", ent->model, ent->lerpflags & LERP_MOVESTEP);
				f = 1;
			}
			...
			ent->origin = Vec_Lerp(ent->msg_origins[1], ent->msg_origins[0], f);
			ent->angles = Vec_LerpAngles(ent->msg_angles[1], ent->msg_angles[0], f);
		...
		if (ent->effects & EF_MUZZLEFLASH)
		{
			...
			dl->origin.z += 16;
			Vec_AngleVectors(ent->angles, &fv, &rv, &uv);

			dl->origin = Vec_MulAdd(dl->origin, 18, fv);
			dl->radius = 200 + (rand()&31);
			dl->minlight = 32;
			dl->die = cl.time + 0.1;
			dl->color = Vec(1, 0.85, 0.7);

			//johnfitz -- assume muzzle flash accompanied by muzzle flare, which looks bad when lerped
			if (r_lerpmodels.value != 2)
			{
				if (ent == &cl_entities[cl.entityindex])
		...
		if (i == cl.entityindex && !chase_active.value)
			continue;
...
int CL_ReadFromServer(void)
{
	...
	if (clua.state)
	{
		QLua_PushLuaFunction(clua.state, &clua.tickfunc);
		QLua_CallFunction(clua.state, 0, LUA_MULTRET);
...
void CL_Tracepos_f(void)
{
	...
	vec3_t v = Vec_MulAdd(r_refdef.vieworg, 8192.0, vpn);
	cltrace_t trace = CL_TraceBox(r_refdef.vieworg, Vec_Zero(), Vec_Zero(), v, NULL);

	
	if (trace.fraction >= 1)
		Con_Printf("Tracepos: trace didn't hit anything\n");
	else
		Con_Printf("Tracepos: (%i %i %i)\n", (int)trace.endpos.x, (int)trace.endpos.y, (int)trace.endpos.z);
}
