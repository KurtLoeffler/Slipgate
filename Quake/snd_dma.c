...
channel_t* SND_PickChannel(int entnum, int entchannel)
{
	...
	for (ch_idx = NUM_AMBIENTS; ch_idx < NUM_AMBIENTS + MAX_DYNAMIC_CHANNELS; ch_idx++)
	{
		...
		if (snd_channels[ch_idx].entnum == cl.entityindex && entnum != cl.entityindex && snd_channels[ch_idx].sfx)
			continue;
...
void SND_Spatialize(channel_t* ch)
{
	...
	source_vec = Vec_Sub(ch->origin, listener_origin);
	dist = Vec_Normalize(&source_vec) * ch->dist_mult;
	dot = Vec_Dot(listener_right, source_vec);
...
static void S_UpdateAmbientSounds(void)
{
	...
	l = Mod_PointInLeaf(*(vec3_t*)listener_origin.values, cl.worldmodel);
...
void S_LocalSound(const char* name)
{
	...
	S_StartSound(cl.entityindex, -1, sfx, Vec_Zero(), 1, 1);
}
