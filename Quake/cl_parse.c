...
void CL_ParseStartSoundPacket(void)
{
	...
	for (i = 0; i < 3; i++)
		pos.values[i] = MSG_ReadCoord(cl.protocolflags);
...
void CL_ParseServerInfo(void)
{
	...
	if (i != PROTOCOL_NETQUAKE && i != PROTOCOL_FITZQUAKE && i != PROTOCOL_RMQ && i != PROTOCOL_SLIPGATE) {
		...
		Host_Error("Server returned version %i, not %i or %i or %i or %i", i, PROTOCOL_NETQUAKE, PROTOCOL_FITZQUAKE, PROTOCOL_RMQ, PROTOCOL_SLIPGATE);
	}
	cl.protocol = i;
	//johnfitz
...
char parsebagpath[256];
void ParseBag(entity_t* ent)
{
	int bagvaluecount = MSG_ReadShort();
	if (bagvaluecount)
	{
		for (int i = 0; i < bagvaluecount; i++)
		{
			...
			{
				float value = MSG_ReadFloat();
				//Con_Printf("\"%s\" = %f", name, value);
				if (clua.state)
				{
					QLua_SetClientReplicatedBagNumber(ent, parsebagpath, name, value);
				}
			}
			else if (valuetype == bagv_boolean)
			{
				bool value = MSG_ReadChar() ? 1 : 0;
				if (clua.state)
				{
					QLua_SetClientReplicatedBagBool(ent, parsebagpath, name, value);
				}
				//Con_Printf("\"%s\" = %s", name, value ? "true" : "false");
			}
			else if (valuetype == bagv_string)
			{
				const char* value = MSG_ReadString();
				if (clua.state)
				{
			...
			{
				int oldlen = strlen(parsebagpath);
				strcat(parsebagpath, ".");
				strcat(parsebagpath, name);
				ParseBag(ent);
				parsebagpath[oldlen] = 0;
...
void CL_ParseUpdate(int bits)
{
	...
	//johnfitz -- PROTOCOL_FITZQUAKE
	if (cl.protocol == PROTOCOL_FITZQUAKE || cl.protocol == PROTOCOL_RMQ || cl.protocol == PROTOCOL_SLIPGATE)
	{
		if (bits & U_EXTEND1)
	...
	if (bits & U_ORIGIN1)
		ent->msg_origins[0].x = MSG_ReadCoord(cl.protocolflags);
	else
		ent->msg_origins[0].x = ent->baseline.origin.x;
	if (bits & U_ANGLE1)
		ent->msg_angles[0].x = MSG_ReadAngle(cl.protocolflags);
	else
		ent->msg_angles[0].x = ent->baseline.angles.x;

	if (bits & U_ORIGIN2)
		ent->msg_origins[0].y = MSG_ReadCoord(cl.protocolflags);
	else
		ent->msg_origins[0].y = ent->baseline.origin.y;
	if (bits & U_ANGLE2)
		ent->msg_angles[0].y = MSG_ReadAngle(cl.protocolflags);
	else
		ent->msg_angles[0].y = ent->baseline.angles.y;

	if (bits & U_ORIGIN3)
		ent->msg_origins[0].z = MSG_ReadCoord(cl.protocolflags);
	else
		ent->msg_origins[0].z = ent->baseline.origin.z;
	if (bits & U_ANGLE3)
		ent->msg_angles[0].z = MSG_ReadAngle(cl.protocolflags);
	else
	...
	{
		byte sbits = MSG_ReadChar();

		ent->id = MSG_ReadLong();

		if (sbits & SGU_MINMAXS)
		{
			ent->mins.x = MSG_ReadCoord(cl.protocolflags);
			ent->mins.y = MSG_ReadCoord(cl.protocolflags);
			ent->mins.z = MSG_ReadCoord(cl.protocolflags);
			ent->maxs.x = MSG_ReadCoord(cl.protocolflags);
			ent->maxs.y = MSG_ReadCoord(cl.protocolflags);
			ent->maxs.z = MSG_ReadCoord(cl.protocolflags);
		}

		ent->solid = MSG_ReadChar();

		if (clua.state)
		{
			parsebagpath[0] = 0;
			strcat(parsebagpath, "rep");
			ParseBag(ent);
		}
	}
...
void CL_ParseBaseline(entity_t* ent, int version) //johnfitz -- added argument
{
	...
	for (i = 0; i < 3; i++)
	{
		ent->baseline.origin.values[i] = MSG_ReadCoord(cl.protocolflags);
		ent->baseline.angles.values[i] = MSG_ReadAngle(cl.protocolflags);
	}
...
void CL_ParseClientdata(void)
{
	...
	{
		if (bits & (SU_PUNCH1<<i))
			cl.punchangle.values[i] = MSG_ReadChar();
		else
			cl.punchangle.values[i] = 0;

		if (bits & (SU_VELOCITY1<<i))
			cl.mvelocity[0].values[i] = MSG_ReadChar()*16;
		else
			cl.mvelocity[0].values[i] = 0;
	}

	//johnfitz -- update v_punchangles
	if (v_punchangles[0].x != cl.punchangle.x || v_punchangles[0].y != cl.punchangle.y || v_punchangles[0].z != cl.punchangle.z)
	{
...
char rpcreaddata[4096];
void CL_ParseRPC()
{
	int size = MSG_ReadShort();
	const char* funcname = MSG_ReadString();
	for (int i = 0; i < size; i++)
	{
		rpcreaddata[i] = (char)MSG_ReadByte();
	}

	if (clua.state)
	{
		rpc_t* rpc = &clua.rpcs[clua.rpccount];
		strcpy(rpc->funcname, funcname);
		memcpy(rpc->data, rpcreaddata, size);
		rpc->size = size;
		clua.rpccount++;
	}
}
...
//
	...
	while (1)
	{
		...
		case svc_setangle:
			for (i = 0; i<3; i++)
				cl.viewangles.values[i] = MSG_ReadAngle(cl.protocolflags);
			break;

		case svc_setview:
			cl.entityindex = MSG_ReadShort();
			break;
