...
mleaf_t* Mod_PointInLeaf(vec3_t p, qmodel_t* model)
{
	...
	while (1)
	{
		if (node->contents < 0)
			return (mleaf_t *)node;
		plane = node->plane;
		d = Vec_Dot(p, plane->normal) - plane->dist;
		if (d > 0)
...
void Mod_LoadTexinfo(lump_t* l)
{
	...
	for (i = 0; i<count; i++, in++, out++)
	{
		...
		len1 = Vec_Length(*(vec3_t*)out->vecs[0]);
		len2 = Vec_Length(*(vec3_t*)out->vecs[1]);
		len1 = (len1 + len2)/2;
		if (len1 < 0.32)
			out->mipadjust = 4;
		else if (len1 < 0.49)
...
void CalcSurfaceExtents(msurface_t* s)
{
	...
	for (i = 0; i<s->numedges; i++)
	{
		...
		for (j = 0; j<2; j++)
		{
			 ...
			 */
			val = ((double)v->position.x * (double)tex->vecs[j][0]) +
				((double)v->position.y * (double)tex->vecs[j][1]) +
				((double)v->position.z * (double)tex->vecs[j][2]) +
				(double)tex->vecs[j][3];
...
void Mod_PolyForUnlitSurface(msurface_t* fa)
{
	...
	for (i = 0; i<numverts; i++)
	{
		...
		*(vec3_t*)poly->verts[i] = vec;
		poly->verts[i][3] = Vec_Dot(vec, *(vec3_t*)fa->texinfo->vecs[0]) * texscale;
		poly->verts[i][4] = Vec_Dot(vec, *(vec3_t*)fa->texinfo->vecs[1]) * texscale;
	}
}
...
void Mod_CalcSurfaceBounds(msurface_t* s)
{
	...
	for (i = 0; i<s->numedges; i++)
	{
		...
		if (s->mins[0] > v->position.x)
			s->mins[0] = v->position.x;
		if (s->mins[1] > v->position.y)
			s->mins[1] = v->position.y;
		if (s->mins[2] > v->position.z)
			s->mins[2] = v->position.z;

		if (s->maxs[0] < v->position.x)
			s->maxs[0] = v->position.x;
		if (s->maxs[1] < v->position.y)
			s->maxs[1] = v->position.y;
		if (s->maxs[2] < v->position.z)
			s->maxs[2] = v->position.z;
	}
}
...
void Mod_LoadPlanes(lump_t* l)
{
	...
	for (i = 0; i<count; i++, in++, out++)
	{
		...
		for (j = 0; j<3; j++)
		{
			out->normal.values[j] = LittleFloat(in->normal[j]);
...
float RadiusFromBounds(vec3_t mins, vec3_t maxs)
{
	...
	for (i = 0; i<3; i++)
	{
		corner.values[i] = fabs(mins.values[i]) > fabs(maxs.values[i]) ? fabs(mins.values[i]) : fabs(maxs.values[i]);
	}

	return Vec_Length(corner);
}
...
void Mod_LoadSubmodels(lump_t* l)
{
	...
	for (i = 0; i<count; i++, in++, out++)
	{
		...
		{ // spread the mins / maxs by a pixel
			out->mins.values[j] = LittleFloat(in->mins.values[j]) - 1;
			out->maxs.values[j] = LittleFloat(in->maxs.values[j]) + 1;
...
void Mod_BoundsFromClipNode(qmodel_t* mod, int hull, int nodenum)
{
	...
	case PLANE_X:
		if (plane->signbits == 1)
			mod->clipmins.x = Min(mod->clipmins.x, -plane->dist - mod->hulls[hull].clip_mins.x);
		else
			mod->clipmaxs.x = Max(mod->clipmaxs.x, plane->dist - mod->hulls[hull].clip_maxs.x);
		break;
	case PLANE_Y:
		if (plane->signbits == 2)
			mod->clipmins.y = Min(mod->clipmins.y, -plane->dist - mod->hulls[hull].clip_mins.y);
		else
			mod->clipmaxs.y = Max(mod->clipmaxs.y, plane->dist - mod->hulls[hull].clip_maxs.y);
		break;
	case PLANE_Z:
		if (plane->signbits == 4)
			mod->clipmins.z = Min(mod->clipmins.z, -plane->dist - mod->hulls[hull].clip_mins.z);
		else
			mod->clipmaxs.z = Max(mod->clipmaxs.z, plane->dist - mod->hulls[hull].clip_maxs.z);
		break;
	default:
...
//
	...
	for (i = 0; i<mod->numsubmodels; i++)
	{
		...
		mod->rmaxs.x = mod->rmaxs.y = mod->rmaxs.z = mod->ymaxs.x = mod->ymaxs.y = mod->ymaxs.z = radius;
		mod->rmins.x = mod->rmins.y = mod->rmins.z = mod->ymins.x = mod->ymins.y = mod->ymins.z = -radius;
		//johnfitz
...
void Mod_CalcAliasBounds(aliashdr_t* a)
{
	...
	for (i = 0; i<3; i++)
	{
		loadmodel->mins.values[i] = loadmodel->ymins.values[i] = loadmodel->rmins.values[i] = 999999;
		loadmodel->maxs.values[i] = loadmodel->ymaxs.values[i] = loadmodel->rmaxs.values[i] = -999999;
		radius = yawradius = 0;
	}
	...
	for (i = 0; i<a->numposes; i++)
		...
		{
			for (k = 0; k<3; k++)
				v.values[k] = poseverts[i][j].v[k] * pheader->scale.values[k] + pheader->scale_origin.values[k];

			for (k = 0; k<3; k++)
			{
				loadmodel->mins.values[k] = Min(loadmodel->mins.values[k], v.values[k]);
				loadmodel->maxs.values[k] = Max(loadmodel->maxs.values[k], v.values[k]);
			}
	...
	radius = sqrt(radius);
	loadmodel->rmins.x = loadmodel->rmins.y = loadmodel->rmins.z = -radius;
	loadmodel->rmaxs.x = loadmodel->rmaxs.y = loadmodel->rmaxs.z = radius;

	//ybounds will be used when entity has nonzero yaw
	yawradius = sqrt(yawradius);
	loadmodel->ymins.x = loadmodel->ymins.y = -yawradius;
	loadmodel->ymaxs.x = loadmodel->ymaxs.y = yawradius;
	loadmodel->ymins.z = loadmodel->mins.z;
	loadmodel->ymaxs.z = loadmodel->maxs.z;
}
...
void Mod_LoadAliasModel(qmodel_t* mod, void* buffer)
{
	...
	for (i = 0; i<3; i++)
	{
		pheader->scale.values[i] = LittleFloat(pinmodel->scale.values[i]);
		pheader->scale_origin.values[i] = LittleFloat(pinmodel->scale_origin.values[i]);
		pheader->eyeposition.values[i] = LittleFloat(pinmodel->eyeposition.values[i]);
	}
