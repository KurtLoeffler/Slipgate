...
{
	Vec_AngleVectors(*((vec3_t*)G_VECTOR(OFS_PARM0)), &pr_global_struct->v_forward, &pr_global_struct->v_right, &pr_global_struct->v_up);
}
...
static void PF_setorigin(void)
{
	...
	e = G_EDICT(OFS_PARM0);
	org = *(vec3_t*)G_VECTOR(OFS_PARM1);
	e->v.origin = org;
	SV_LinkEdict(e, false);
}


static void SetMinMaxSize(edict_t* e, vec3_t minvec, vec3_t maxvec, bool rotate)
{
	//float* angles;
	vec3_t rmin, rmax;
	float bounds[2][3];
	...
	for (i = 0; i < 3; i++)
		if (minvec.values[i] > maxvec.values[i])
	...
	else
	{
		...
		*(vec3_t*)bounds[0] = minvec;
		*(vec3_t*)bounds[1] = maxvec;

		rmin.x = rmin.y = rmin.z = 9999;
		rmax.x = rmax.y = rmax.z = -9999;
		...
		for (i = 0; i <= 1; i++)
		{
			...
			for (j = 0; j <= 1; j++)
			{
				...
				for (k = 0; k <= 1; k++)
				{
					...
					// transform the point
					transformed.x = xvector[0]*base.x + yvector[0]*base.y;
					transformed.y = xvector[1]*base.x + yvector[1]*base.y;
					transformed.z = base.z;

					for (l = 0; l < 3; l++)
					{
						if (transformed.values[l] < rmin.values[l])
							rmin.values[l] = transformed.values[l];
						if (transformed.values[l] > rmax.values[l])
							rmax.values[l] = transformed.values[l];
					}
				}
			}
		}
	...
	e->v.mins = rmin;
	e->v.maxs = rmax;
	e->v.size = Vec_Sub(maxvec, minvec);
...
static void PF_setsize(void)
{
	...
	e = G_EDICT(OFS_PARM0);
	minvec = *(vec3_t*)G_VECTOR(OFS_PARM1);
	maxvec = *(vec3_t*)G_VECTOR(OFS_PARM2);
...
static void PF_setmodel(void)
{
	...
	//johnfitz
		...
		SetMinMaxSize(e, Vec_Zero(), Vec_Zero(), true);
}
...
static void PF_normalize(void)
{
	...
	value1 = (float*)G_VECTOR(OFS_PARM0);

	new_temp = value1[0] * value1[0] + value1[1] * value1[1] + value1[2]*value1[2];
	new_temp = sqrt(new_temp);

	if (new_temp == 0)
		newvalue.x = newvalue.y = newvalue.z = 0;
	else
	{
		new_temp = 1 / new_temp;
		newvalue.x = value1[0] * new_temp;
		newvalue.y = value1[1] * new_temp;
		newvalue.z = value1[2] * new_temp;
	}
...
static void PF_traceline(void)
{
	...
	if (developer.value) {
		if (IS_NAN(v1.x) || IS_NAN(v1.y) || IS_NAN(v1.z) ||
			IS_NAN(v2.x) || IS_NAN(v2.y) || IS_NAN(v2.z)) {
			Con_Warning("NAN in traceline:\nv1(%f %f %f) v2(%f %f %f)\nentity %d\n",
				v1.x, v1.y, v1.z, v2.x, v2.y, v2.z, NUM_FOR_EDICT(ent));
		}
	}

	if (IS_NAN(v1.x) || IS_NAN(v1.y) || IS_NAN(v1.z))
		v1.x = v1.y = v1.z = 0;
	if (IS_NAN(v2.x) || IS_NAN(v2.y) || IS_NAN(v2.z))
		v2.x = v2.y = v2.z = 0;

	trace = SV_Move(v1, Vec_Zero(), Vec_Zero(), v2, nomonsters, ent);
...
static int PF_newcheckclient(int check)
{
	...
	org = Vec_Add(ent->v.origin, ent->v.view_ofs);
	leaf = Mod_PointInLeaf(*(vec3_t*)org.values, sv.worldmodel);
...
static void PF_checkclient(void)
{
	...
	view = Vec_Add(self->v.origin, self->v.view_ofs);
	leaf = Mod_PointInLeaf(*(vec3_t*)view.values, sv.worldmodel);
...
static void PF_findradius(void)
{
	...
	for (i = 1; i < sv.num_edicts; i++, ent = NEXT_EDICT(ent))
	{
		...
		if (ent->v.solid == SOLID_NOT)
			continue;
		for (j = 0; j < 3; j++)
			eorg.values[j] = org.values[j] - (ent->v.origin.values[j] + (ent->v.mins.values[j] + ent->v.maxs.values[j]) * 0.5);
		if (Vec_Length(eorg) > rad)
			continue;
...
static void PF_vtos(void)
{
	...
	s = PR_GetTempString();
	sprintf(s, "'%5.1f %5.1f %5.1f'", G_VECTOR(OFS_PARM0)->x, G_VECTOR(OFS_PARM0)->y, G_VECTOR(OFS_PARM0)->z);
	G_INT(OFS_RETURN) = PR_SetEngineString(s);
}
...
static void PF_aim(void)
{
	...
	end = Vec_MulAdd(start, 2048, dir);
	tr = SV_Move(start, Vec_Zero(), Vec_Zero(), end, false, ent);
	...
	for (i = 1; i < sv.num_edicts; i++, check = NEXT_EDICT(check))
	{
		...
		for (j = 0; j < 3; j++)
			end.values[j] = check->v.origin.values[j] + 0.5 * (check->v.mins.values[j] + check->v.maxs.values[j]);
		dir = Vec_Sub(end, start);
		Vec_Normalize(&dir);
		dist = Vec_Dot(dir, pr_global_struct->v_forward);
		if (dist < bestdist)
	...
	if (bestent)
	{
		dir = Vec_Sub(bestent->v.origin, ent->v.origin);
		dist = Vec_Dot(dir, pr_global_struct->v_forward);
		end = Vec_Scale(pr_global_struct->v_forward, dist);
		end.z = dir.z;
		Vec_Normalize(&end);
...
void PF_changeyaw(void)
{
	...
	ent = PROG_TO_EDICT(pr_global_struct->self);
	current = AngleMod(ent->v.angles.y);
	...
	ent->v.angles.y = AngleMod(current + move);
}
...
static void PF_makestatic(void)
{
	...
	for (i = 0; i < 3; i++)
	{
		MSG_WriteCoord(&sv.signon, ent->v.origin.values[i], sv.protocolflags);
		MSG_WriteAngle(&sv.signon, ent->v.angles.values[i], sv.protocolflags);
	}
